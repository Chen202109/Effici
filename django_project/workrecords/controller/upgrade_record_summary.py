...
def analysis_service_upgrade_trend(request):
    if request.method == 'GET':
        ...
        data = [{'service': f"{resource_pool}-{function_name}",
                 'data': upgrade_summary_service.get_service_upgrade_trend(begin_date, end_date, function_name, resource_pool)} for function_name in
                function_names for resource_pool in resource_pools]
...
def analysis_version_problem_by_resource_pool(request):
    ...
    if request.method == 'GET':
        ...
        for resource_pool in resource_pools:
            ...
            data.extend(get_work_record_resource_pool_error_function_summary(begin_date, end_date, resource_pool, function_names, saas_version_data))
