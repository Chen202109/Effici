...
# 因此拼接好的字段英文名，数据列表
def any_inster_sql(filePath,name_field,data_title,sql_key_head='',sql_value_head=''):
    ...
    sql_key_str = 'fid,'+ sql_key_head # 保存拼接成的sql字段, ，先加固定的主键FID,eventtype(重大故障),environment(私有化)
    sql_key_col = [] # 保存sql字段所在的数据是第几列
    # sql_value_str 在for循环结束的时候要初始一次，否则第二次排序就不对
    sql_value_str ="'"+''.join(str(uuid.uuid4()).split('-'))+"'," + sql_value_head # 每一行的value数值内容，第一个数值为上面sql_key_str对应的FID
        ...
        if i>0: #vlaue数据从第二行开始，# 2. 获取inster into 的字段名
            for n in range(len(sql_key_col)):
                ...
                if sheet.cell_value(i, sql_key_col[n]) == '': # 当取到的日期 是空的，那么就赋值成空值
                    ...
                    if data_title in sheet.cell_value(0,sql_key_col[n]): # 判断哪个列标题下数据要转换成时间格式。如果是 某某日期，那么需要用xldate_as_datetime将float转成日期
                        ...
                        if '编码' in sheet.cell_value(0, sql_key_col[n]):
                            cell_value = str(sheet.cell_value(i, sql_key_col[n]))[:-2]  #有财政编码，单位编码，区划编码，会取出150000.0 或 00012345.0，要进行去除
                        else:
                            cell_value = str(sheet.cell_value(i, sql_key_col[n]))

                        sql_value_str = sql_value_str + "'" + cell_value.replace("'", "").replace('"', '') + "'" + ','   #去掉里面的sql影响字符
            ...
            sql_value_str ="'"+''.join(str(uuid.uuid4()).split('-')) + "'," + sql_value_head  # 组装完一行数据，就清空一下
...
if __name__ == '__main__':
    ...
    file_path = f'C:\\Users\\Administrator\\Downloads\\监控异常导入文件123.xlsx'
    ...
    any_inster_sql(file_path,name_field)